...
test.describe('US4.4 - Edit Lease', () => {
  let page: Page;
  ...
  test('TC-E2E-001: Edit lease successfully', async () => {
    // Click edit button
    const editButton = page.locator('button:has-text("עריכה"), button[aria-label*="edit"]').first();
    ...
    // Submit
    const saveButton = page.locator('button:has-text("עדכן"), button[type="submit"]').last();
    ...
    await expect(page.locator('text=6000').first()).toBeVisible({ timeout: 10000 });
  });

  test('TC-E2E-002: Cannot edit terminated lease', async () => {
    // Terminate the lease first
    await fetch(`${BACKEND_URL}/leases/${TEST_LEASE_ID}/terminate`, { method: 'POST' });
    await page.reload();
    await page.waitForLoadState('networkidle');
    
    // Try to edit - should not be possible or should show error
    const editButton = page.locator('button:has-text("עריכה")').first();
