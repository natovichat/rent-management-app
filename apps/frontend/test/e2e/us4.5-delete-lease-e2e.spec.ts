...
test.describe('US4.5 - Delete Lease', () => {
  let page: Page;
  ...
  test('TC-E2E-001: Delete lease with confirmation', async () => {
    // Click delete button
    const deleteButton = page.locator('button:has-text("מחיקה"), button[aria-label*="delete"]').first();
    await deleteButton.click();
    
    // Confirm deletion
    await page.waitForSelector('[role="dialog"]', { timeout: 5000 });
    const confirmButton = page.locator('button:has-text("מחק"), button:has-text("אישור")').last();
    await confirmButton.click();
    
    // Wait for deletion
    await page.waitForTimeout(2000);
    
    // Verify lease no longer appears
    await expect(page.locator('text=יוחנן כהן').first()).not.toBeVisible({ timeout: 5000 });
  });
});
