...
/**
 * Component for Owner CSV import/export actions.
 */
export default function OwnerCsvActions() {
  ...
  const handleDownloadExample = async () => {
    handleMenuClose();
    try {
      const response = await api.get('/owners/csv/example', {
        responseType: 'blob',
      });
      ...
      link.href = url;
      link.download = 'owners-example.csv';
  ...
  const importMutation = useMutation({
    ...
    onError: (error: any) => {
      alert(error.response?.data?.message || 'Failed to import owners');
    },
  });
    ...
    <>
      ...
      {(importMutation.isPending || previewMutation.isPending) && (
        ...
        >
          <CircularProgress />
          <Typography sx={{ mt: 2 }}>
            {previewMutation.isPending ? 'מציג תצוגה מקדימה...' : 'מייבא בעלים...'}
          </Typography>
        </Box>
      )}
        ...
        <DialogContent>
          {importResult && (
            <>
              ...
              >
                <Typography>
                  <strong>הצלחה:</strong> {importResult.success} בעלים
                </Typography>
                <Typography>
                  <strong>כשלון:</strong> {importResult.failed} בעלים
                </Typography>
              </Alert>
